...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSObject *model))completion;
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<NSObject *> *models))completion;

/// Get Assets 获得Asset数组
- (void)getAssetsFromFetchResult:(id)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<NSObject *> *models))completion;
- (void)getAssetFromFetchResult:(id)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSObject *model))completion;

/// Get photo 获得照片
- (void)getPostImageWithAlbumModel:(NSObject *)model completion:(void (^)(UIImage *postImage))completion;
...
/**
 获得视频，若视频本地不村子啊，存在云端，则从云端下载

 @param asset PHAsset
 @param progress 云端下载进度
 @param completion 完成回调
 */
- (NSInteger)getVideoWithAsset:(id)asset progress:(void(^)(NSError * error, CGFloat progress))progress completion:(void (^)(AVPlayerItem * playerItem, NSDictionary * info))completion;

/**
 取消从云端下载视频

 @param downloadTaskId 下载taskID
 */
- (void)cancleDownloadingVideoWithTaskId:(NSInteger)downloadTaskId;
