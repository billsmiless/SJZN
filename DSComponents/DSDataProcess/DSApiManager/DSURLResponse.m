...
@property (nonatomic, assign, readwrite) DSURLResponseStatus status;
...
- (instancetype)initWithResponseString:(NSString *)responseString requestId:(NSNumber *)requestId request:(NSURLRequest *)request responseData:(NSData *)responseData error:(NSError *)error
{
    self = [super init];
    if (self) {
        self.contentString = [responseString ds_defaultValue:@""];
...
- (DSURLResponseStatus)responseStatusWithError:(NSError *)error
{
    if (error) {
        DSURLResponseStatus result = DSURLResponseStatusErrorNoNetwork;
        
        // 除了超时以外，所有错误都当成是无网络
        if (error.code == NSURLErrorTimedOut) {
            result = DSURLResponseStatusErrorTimeout;
        }
        return result;
    } else {
        return DSURLResponseStatusSuccess;
    }
}
